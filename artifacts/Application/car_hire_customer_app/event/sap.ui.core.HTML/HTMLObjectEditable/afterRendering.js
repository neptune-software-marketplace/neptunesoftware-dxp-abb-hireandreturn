...
function _handlePathClick(oEvent) {
    ...
    var severityValue = modelSelectedCar.getData().parts[$(oEvent.target).attr("id")].severity;
    ...
    if (modelSelectedCar.getData().parts[$(oEvent.target).attr("id")].type == "dent") {
        Dent.addStyleClass("selectedProblem");
    }
    if (modelSelectedCar.getData().parts[$(oEvent.target).attr("id")].type == "scratch") {
        Scratch.addStyleClass("selectedProblem");
    }
    if (modelSelectedCar.getData().parts[$(oEvent.target).attr("id")].type == "chip") {
...
// console.log(modelSelectedCar.getData().parts.path1.type);
